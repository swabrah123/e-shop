...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Get images array using your helper function
...
?>
 ...
 <li class="breadcrumb-item">
  <a class="text-nowrap" href="<?= url('index.php') ?>"><i class="ci-home"></i>Home</a>
</li>
...
<li class="breadcrumb-item text-nowrap active" aria-current="page">
  <?= $product ? htmlspecialchars($product['name']) : 'Product' ?>
</li>
              </ol>
            </nav>
          </div>
        ...
        <div class="product-gallery">
          <div class="product-gallery-preview order-sm-2">
            <?php foreach ($images as $index => $img): ?>
              <div class="product-gallery-preview-item <?= $index === 0 ? 'active' : '' ?>" id="<?= image_id($index) ?>">
<img
  <?= $index === 0 ? 'id="main-image"' : '' ?>
  class="image-zoom"
  src="<?= htmlspecialchars($img) ?>"
  data-zoom-image="<?= htmlspecialchars($img) ?>"
  alt="Product image"
>
          ...
          <div class="product-gallery-thumblist order-sm-1">
            <?php foreach ($images as $index => $img): ?>
              <?php
              // Replace gallery folder with thumbnail folder if needed
              $thumb = str_replace('/gallery/', '/gallery/th', $img);
              ?>
              <a class="product-gallery-thumblist-item <?= $index === 0 ? 'active' : '' ?>" href="#<?= image_id($index) ?>">
  <img class="thumbnail-image" src="<?= htmlspecialchars($thumb) ?>" alt="Product thumb">
</a>
      ...
      <div class="col-lg-5 pt-4 pt-lg-0">
        <div class="product-details ms-auto pb-3">
          ...
          <div class="mb-3">
            <span class="h3 fw-normal text-accent me-1">$<?= htmlspecialchars(number_format($product['selling_price'] ?? 0, 2)) ?></span>
            <?php if (!empty($product['buying_price'])): ?>
            <del class="text-muted fs-lg me-3">$<?= htmlspecialchars(number_format($product['buying_price'], 2)) ?></del>
            <?php endif; ?>
            <span class="badge bg-danger badge-shadow align-middle mt-n2">Sale</span>
          </div>
          <div class="fs-sm mb-4">
            <span class="text-heading fw-medium me-1">Color:</span><span class="text-muted" id="colorOption"><?= htmlspecialchars($product['color'] ?? 'Red/Dark blue/White') ?></span>
          </div>

          <!-- Colors selection (static example, can be dynamic if you have colors array) -->
          <div class="position-relative me-n4 mb-3">
            <!-- ... your color options here ... -->
          </div>

          <form action="cart-process-add.php"   class="mb-grid-gutter" method="get">
          ...
          <!-- Rest of product info (composition, shipping, etc.) static or can be dynamic -->
