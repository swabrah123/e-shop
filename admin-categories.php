...
<!-- Page Title-->
  ...
  <img 
    src="<?= htmlspecialchars($category['photo']) ?>" 
  ...
  >
      ...
      <div class="text-center text-sm-start">
        ...
        <?php if (!empty($category['description'])): ?>
          <p class="mb-2 text-muted"><?= htmlspecialchars($category['description']) ?></p>
        <?php endif; ?>

        <div class="d-inline-block text-muted fs-ms">
          Created: <span class="fw-medium"><?= date('M d, Y', strtotime($category['created_at'])) ?></span>
        </div>

        <div class="d-flex justify-content-center justify-content-sm-start pt-3">
          <!-- Download Button -->
          <a href="<?= htmlspecialchars($category['photo']) ?>" download class="btn bg-faded-accent btn-icon me-2" data-bs-toggle="tooltip" title="Download">
            <i class="ci-download text-accent"></i>
          </a>

          <!-- Edit Button -->
          <a href="admin-categories-edit.php?id=<?= $category['id'] ?>" class="btn bg-faded-info btn-icon me-2" data-bs-toggle="tooltip" title="Edit">
            <i class="ci-edit text-info"></i>
          </a>

          <!-- Delete Button -->
          <a href="admin-categories-delete.php?id=<?= $category['id'] ?>" class="btn bg-faded-danger btn-icon" data-bs-toggle="tooltip" title="Delete" onclick="return confirm('Are you sure you want to delete this category?');">
  ...
  <?php endforeach; ?>
<?php else: ?>
  <div class="py-4 text-center text-muted">No categories found.</div>
<?php endif; ?>
